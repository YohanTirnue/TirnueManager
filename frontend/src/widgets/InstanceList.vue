          ...
          <fade-up-animation>
            ...
            >
              <Shortcut
                ...
                :daemon-name="showAllDaemons
                  ? ((item as any).daemonRemarks || `${(item as any).daemonIp || 'unknown'}:${(item as any).daemonPort || '?'}`)
                  : (currentRemoteNode?.remarks || `${currentRemoteNode?.ip || 'unknown'}:${currentRemoteNode?.port || '?'}`)"
