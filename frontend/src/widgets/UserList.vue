...
const { execute: executeGetAllInstances } = remoteAllInstances();
...
const handleAssignInstance = () => {
  ...
  const alreadyAssigned = formData.value.instances?.some(
    (inst: any) =>
  ...
  if (!formData.value.instances) {
    formData.value.instances = [];
  }

  formData.value.instances.push({
...
const handleRemoveInstance = (instanceUuid: string, daemonId: string) => {
  Modal.confirm({
    ...
    onOk: () => {
      if (formData.value.instances) {
        formData.value.instances = formData.value.instances.filter(
...
const getInstanceName = (instanceUuid: string, daemonId: string) => {
  // This would ideally fetch from instance details, but for now we'll show a truncated UUID
    ...
    >
      ...
      <div v-if="currentUserPermission >= 10" class="user-settings-card">
        <div class="section-header-industrial">
          ...
          <div class="section-title">
            <h4>Assigned Instances ({{ formData.instances?.length || 0 }})</h4>
            <span>Direct instance assignments with permissions</span>
          </div>
          <button
            type="button"
            class="btn-add-daemon"
            @click="showAssignInstanceModal()"
          >
            <PlusOutlined />
            Assign Instance
          </button>
        </div>
        <div v-if="formData.instances && formData.instances.length > 0" class="owned-daemons-grid">
          <div
            v-for="instance in formData.instances"
          ...
          >
            <div class="daemon-header">
              ...
              <div class="daemon-info">
                <div class="daemon-name">{{ instance.instanceInfo?.config?.nickname || getInstanceName(instance.instanceUuid, instance.daemonId) }}</div>
                <div class="daemon-id">UUID: {{ instance.instanceUuid.substring(0, 8) }}...</div>
              </div>
            </div>
            ...
            <div class="daemon-actions">
              <button
                ...
                @click="handleRemoveInstance(instance.instanceUuid, instance.daemonId)"
              >
                <DeleteOutlined />
                Remove
              </button>
    ...
    <div class="owned-daemon-modal-content">
      <a-form layout="vertical">
        <a-form-item label="Daemon/Node" required>
          ...
          <div class="field-hint" style="margin-top: 8px;">
            Select the node where the instance is located
          </div>
        </a-form-item>
        ...
        <a-form-item label="Instance" required>
          ...
          >
            ...
            >
              ...
              <span style="color: #999; margin-left: 8px;">
                ({{ instance.status === 'running' ? 'Running' : instance.status === 'stopped' ? 'Stopped' : 'Unknown' }})
              </span>
            </a-select-option>
          </a-select>
          <div class="field-hint" style="margin-top: 8px;">
            {{ instanceModal.selectedDaemon ? 'Choose which instance to assign' : 'Select a daemon first' }}
          </div>
        </a-form-item>
        ...
        <a-form-item label="RAM Allocation (MB)">
          <div class="limit-input-group">
            ...
            <div class="ram-presets">
              <button
                type="button"
                class="btn-preset"
                @click="instanceModal.ramAllocatedMB = 512"
              >
                512MB
              </button>
              <button
                type="button"
                class="btn-preset"
                @click="instanceModal.ramAllocatedMB = 1024"
              >
                1GB
              </button>
              <button
                type="button"
                class="btn-preset"
                @click="instanceModal.ramAllocatedMB = 2048"
              >
                2GB
              </button>
            </div>
          ...
          <div class="field-hint" style="margin-top: 8px;">
            Optional: Track RAM allocated to this user for this instance
          </div>
        </a-form-item>
          ...
          <div class="limit-input-group">
            ...
            <div class="ram-presets">
              <button
                type="button"
                class="btn-preset"
                @click="instanceModal.maxSubUsers = 3"
              >
                Default (3)
              </button>
              <button
                type="button"
                class="btn-preset"
                @click="instanceModal.maxSubUsers = 5"
              >
                5
              </button>
              <button
                type="button"
                class="btn-preset"
                @click="instanceModal.maxSubUsers = 10"
              >
                Max (10)
              </button>
            </div>
    ...
    <template #footer>
      <div class="modal-footer-industrial">
        <button
          type="button"
          class="btn-cancel-industrial"
          @click="instanceModal.visible = false"
        >
          Cancel
        </button>
        <button
        ...
        >
          <span v-if="instanceModal.loading">Assigning...</span>
          <span v-else>Assign Instance</span>
        </button>
      </div>
    </template>
  </a-modal>
